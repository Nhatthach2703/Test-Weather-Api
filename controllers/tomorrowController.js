 ...
 * @returns {object|null} - Tọa độ {lat, lon} hoặc null nếu không tìm thấy
 */
...
/**
 * Controller lấy thời tiết hiện tại
...
const getCurrentWeather = async (req, res) => {
  try {
    ...
    console.error('Error fetching current weather:', err.response?.data || err.message);
    res.status(500).json({ error: 'Failed to fetch current weather' });
  }
};

/**
 * Controller lấy dự báo thời tiết theo ngày
 * @param {object} req - Request object
 * @param {object} res - Response object
 */
const getForecastWeather = async (req, res) => {
  try {
    ...
    console.error('Error fetching forecast weather:', err.response?.data || err.message);
